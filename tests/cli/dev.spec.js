...
describe("test dev command", () => {
  ...
  it("check dev log output", async () => {
    ...
    await assertAndKillChildProcess(dev, async () => {
      expect(output).to.contain("➜  Local:   http://localhost:");
    });
  });
  ...
  it("check --port flag", async () => {
    ...
    await assertAndKillChildProcess(dev, async () => {
      expect(output).to.contain("➜  Local:   http://localhost:3000");
    });
  });
  ...
  it("check --host flag", async () => {
    ...
    await assertAndKillChildProcess(dev, async () => {
      expect(output).toMatch(containsIP);
    });
  });

  it("check --publicDir flag", async () => {
    const testPublic = fileURLToPath(
      new URL("../../testPublic", import.meta.url),
    );
    await mkdir(testPublic, { recursive: true }).catch((e) => {
      console.error(e);
    });
    await writeFile(path.join(testPublic, "test.txt"), "test file").catch(
      (e) => {
        console.error(e);
      },
    );
    ...
    await assertAndKillChildProcess(
      dev,
      async () => {
        const text = await axios
          .get("http://localhost:5173/test.txt")
          .then((resp) => resp.data)
          .catch((e) => {
            console.error(e);
          });
  ...
  it("test --runtime flag", async () => {
    const runtimeFile = fileURLToPath(
      new URL("../../testRuntime.js", import.meta.url),
    );
    await writeFile(
      runtimeFile,
      `export default ${JSON.stringify(mockedEodash)}`,
    );
    ...
    await assertAndKillChildProcess(
      dev,
      async () => {
        const runtimeStr = await axios
          .get("http://localhost:5173/@fs/config.js")
          .then((resp) => resp.data)
          .catch((e) => {
            console.error(e);
          });
